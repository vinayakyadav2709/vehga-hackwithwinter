...
const MapView: React.FC<MapViewProps> = ({ junctions, junctionStatuses, predictions, events }) => {
  // ✅ Fix default Leaflet marker icons using CDN URLs (production-safe with bundlers)
  useEffect(() => {
  ...
  const getJunctionStatus = (junctionId: number): JunctionStatus | undefined =>
    junctionStatuses.find((status) => status.junction_id === junctionId);

  const getPrediction = (junctionId: number): Prediction | undefined =>
    predictions.find((pred) => pred.junction_id === junctionId);

  // ✅ pre-filter once (cleaner + avoids repeated guards)
  ...
  return (
    <div className="h-full w-full">
      <MapContainer center={[12.9716, 77.5946]} zoom={12} className="h-full w-full" zoomControl>
        <TileLayer
                          ...
                          <p>
                            ...
                            <span
                              className={`capitalize ${
                                getStatusColor(junction.status) === 'green'
                                  ? 'text-green-600'
                                  : getStatusColor(junction.status) === 'red'
                                  ? 'text-red-600'
                                  : getStatusColor(junction.status) === 'yellow'
                                  ? 'text-yellow-600'
                                  : 'text-gray-600'
                              }`}
                            >
                              {junction.status?.replace('_', ' ') || 'unknown'}
                            </span>
                          </p>

                          <p>
                            <strong>Lanes:</strong> {junction.lanes || 'N/A'}
                          </p>
                          ...
                          {status && (
                            <>
                              <p>
                                ...
                                {formatWaitTime(status.average_wait_time_sec)}
                              </p>
                              <p>
                                <strong>Emergency Priority:</strong>{' '}
                                {status.emergency_vehicle_priority ? 'Active' : 'Inactive'}
                              </p>

                              <div className="mt-2">
                                <p className="font-semibold">Traffic Lights:</p>
                                <div className="grid grid-cols-2 gap-1 mt-1">
                                  {status.traffic_lights?.length ? (
                      ...
                      <div className="p-2 min-w-[200px]">
                        ...
                        <div className="space-y-1">
                          <p>
                            ...
                            <span
                              className={`capitalize ml-1 font-semibold ${
                                prediction.predicted_congestion === 'light'
                                  ? 'text-green-600'
                          ...
                          <p>
                            ...
                            {formatWaitTime(prediction.expected_wait_time_sec || 0)}
                          </p>
                        </div>
                      </div>
                    </Popup>
